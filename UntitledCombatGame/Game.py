...
hero1_image = pygame.image.load('assets/Characters/HeroKnight/Idle/0.png').convert_alpha()
...
start_button = AlternativeButton.AlternativeButton(567, 50, start_image, 0.5)
char_scale = 2
hitboxes = []
health_list = []
attack_player = None
...
def champ_button(image, x, y):
    return AlternativeButton.AlternativeButton(x, y, image, char_scale)
...
hero1_button_image = pygame.transform.scale(hero1_image, (int(hero1_image.get_width()),
                                                          int(hero1_image.get_height())))
...
champ1_button = champ_button(hero1_button_image, 321, 375)
bandit_button = champ_button(bandit_button_image, 595, 380)
numan_button = champ_button(numan_button_image, 837, 378)
hero1_image = pygame.transform.scale(hero1_image, (int((hero1_image.get_width() * 4) / 2),
                                                   int((hero1_image.get_height() * 4) / 2)))
...
hero_knight_champ = Champ('HeroKnight', hero1_image, char_scale, False)
bandit = Champ('bandit', bandit_image, char_scale, False)
light_bandit_champ = Champ('Light Bandit', numan_image, char_scale, False)
champ_list = [hero_knight_champ, bandit, light_bandit_champ]
...
def select_screen(first_player_name, second_player_name):
    ...
    while run:
        ...
        if numan_button.draw(Screen):
            ...
            hero_knight_champ.selected = False
            bandit.selected = False
            light_bandit_champ.selected = True
            temp = 2
...
def leaderBoard():
    ...
    while run:
        pygame.display.set_caption("Main Menu")
        showLeaderBoard()
        back_button1 = AlternativeButton.AlternativeButton(32, 16, ok_img, 1)
